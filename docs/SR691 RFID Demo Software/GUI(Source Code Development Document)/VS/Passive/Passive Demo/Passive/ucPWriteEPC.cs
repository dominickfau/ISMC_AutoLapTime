...
namespace ADSDKDemo
{
    public partial class ucPWriteEPC : UserControl
    {
        ...
        private void ltxtCardDecimal_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                blnChangeFlag = true;
                LngSendData = ConvertData.DecStringToDecLong(ltxtCardDecimal.Text.Trim(), IniSettings.WriteMode);
        ...
        private void utxtCardHex_TextChanged(object sender, EventArgs e)
        {
            if (!blnChangeFlag)
            {
                ...
                if(chkAddHex.Checked)
                    LngSendData = Convert.ToUInt64(utxtCardHex.Text.Replace("-", ""));
                else
                    LngSendData = ConvertData.HexStringToDecLong(utxtCardHex.Text.Replace("-", ""), IniSettings.WriteMode);
        ...
        private void utxtCardWg_TextChanged(object sender, EventArgs e)
        {
            if (!blnChangeFlag)
            {
                ...
                if (IniSettings.WriteMode == 1)
                {
                    if (lngfirst > 65535) lngfirst = 65535;
                }
                else if (IniSettings.WriteMode == 0)
                {
        ...
        private void LoadQuickWritePara()
        {
            ...
            try
            {
                LngSendData = Convert.ToUInt64(IniSettings.WriteCard);
            }
            catch { }
            RefrashCard();
        }
        ...
        /// Already Send Data
        /// </summary>
        ulong LngSendDataP = 0;
        /// <summary>
        /// Recv Data
        /// </summary>
        ulong LngRecvData = 0;

        public string ShowCard(ulong Card, bool isHex)
        {
            if (isHex)
            {
                if (chkAddHex.Checked)
                ...
                {
                    return ConvertData.DecLongToHexString(Card, IniSettings.WriteMode);
                }
            }
            else
            {
        ...
        private void GetCard(byte[] data)
        {
            if (chkAddHex.Checked)
            {
                LngRecvData = Convert.ToUInt64(ConvertData.ByteArrayToHexString(data, 1 + intpoint, (IniSettings.WriteMode > 0?4:3)));
            }
            else
            {
                if (IniSettings.WriteMode > 0)
                {
                    LngRecvData = (ulong)data[1 + intpoint] << 24;
                    LngRecvData += (ulong)data[2 + intpoint] << 16;
                    LngRecvData += (ulong)data[3 + intpoint] << 8;
                    LngRecvData += (ulong)data[4 + intpoint];
                }
                else
                {
                    LngRecvData = (ulong)data[1 + intpoint] << 16;
                    LngRecvData += (ulong)data[2 + intpoint] << 8;
                    LngRecvData += (ulong)data[3 + intpoint];
                }
            }
        ...
        private void RefrashCard()
        {
            ...
            ulong lngfirst = LngSendData / 65536;
            ulong lngsecond = LngSendData % 65536;
        ...
        private void WriteCard()
        {
            ...
            try
            {
                byte[] SendDataBuff = ConvertData.DecLongToByteArray(LngSendData, IniSettings.WriteMode);
                ...
                {
                    SendDataBuff = ConvertData.DecLongToByteArray(LngSendData, IniSettings.WriteMode);
                }
