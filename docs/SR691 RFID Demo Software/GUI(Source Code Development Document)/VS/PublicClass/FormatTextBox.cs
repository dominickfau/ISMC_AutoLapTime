...
namespace ADSDK.Bases.Components
{
    ...
    public partial class FormatTextBox : TextBox
    {
        ...
        private const string DATE = "0000-00-00";
                ...
                if (!m_stdmaskChg)
                {
                    ...
                    switch (value)
                    {
                        ...
                        case TEL:
                            ...
                            break;
                        case MOBILE:
                            StdInputMask = InputMaskType.MOBILE;
                            break;
                        case DATE:
                            StdInputMask = InputMaskType.DATE;
                            break;
                        default:
        ...
        private Hashtable RegExps
        {
            get
            {
                if (m_regexps == null)
                {
                    ...
                    m_regexps.Add('6', @"[0-6]");		// digit required
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            switch (keyData)
            {
                ...
                case Keys.C | Keys.Control:
                    Clipboard.SetText(base.Text, TextDataFormat.Text);
                    return true;
        ...
        protected override void OnKeyPress(KeyPressEventArgs e)
        {
            ...
            string keychar = GetDBCCase(e.KeyChar.ToString());
            // update display if valid char entered
            //if (IsValidChar(e.KeyChar, (int)m_posNdx[strt]))
            if (IsValidString(keychar, (int)m_posNdx[strt]))
            {
                // assemble new text
                string t = "";
        ...
        private bool IsValidString(string input, int pos)
        {
            // validate input string against mask
            return Regex.IsMatch(input, (string)RegExps[InputMask[pos]]);
        }
        ...
        private bool IsMaskString(string input)
        {
            // check string
            return Regex.IsMatch(input, (string)RegExps[MASK_KEY]);
        }
        ...
        /// <returns></returns>
        public string GetDBCCase(string inputString)
        {
            if (2 * inputString.Length == Encoding.Default.GetByteCount(inputString))
            {
                return SBCToDBCCase(inputString);
            }
            else
            {
                return inputString;
        ...
        ///<returns></returns>
        public static string SBCToDBCCase(string SBCCase)
        {
